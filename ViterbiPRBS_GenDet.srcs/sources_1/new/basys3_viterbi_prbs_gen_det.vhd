...
--  bufr_clk: bufr
--    generic map (
--      BUFR_DIVIDE => "1", --this should divide by 2 giving 50MHz clock
--      SIM_DEVICE  => "7SERIES"
--    )
--    port map(
--        O   => clk1,
...
--        I   => clk);
  ...
  vit_encoder : entity work.fdfwd_conv_enc
    ...
    port map (
      ...
      bits_out   => bits_out,  --bits_out1(0 to 1)
      valid_data => valid_data_out,--valid_data,
      word_start => word_start_out,--word_start1,
      ready      => encoder_rdy);
  ...
  vit_decoder : entity work.fdfwd_viterbi_dec
    ...
    port map (
      ...
      gen_poly1   => gen_poly1,
      gen_poly2   => gen_poly2,
      valid_data  => valid_data_in,--valid_data,
      word_start  => word_start_in,--word_start1,
      bits_in     => bits_in,--bits_out1(0 to 1)
